...
effet_eclair =  effet([chevalier.droit_orientation], "rien", "orientez-vous")
...
def actions_possibles(joueur, pile):
    """
    renvoie la liste des actions possibles d'un joueur en fonction des cartes dans sa main et de la vitesse de la pile
    :return possibles: liste, liste des actions possibles
    Auteur: Léa Vilasi
    """
    possibles = []
    if pile.vitesse < 1:
...
def choix(joueur, adversaire, possible, pile):
    ...
    :return choix: carte, la carte jouée par l'IA (si elle décide de jouer)
    Auteur: Léa Vilasi
    """
    choix = carte_init
    bouge = 0
    if joueur.peut_orienter == 1:
        if (adversaire.pion.x - joueur.pion.x) >= 100:
            joueur.pion.orienter("est")
            joueur.peut_orienter = 0
        elif (adversaire.pion.x - joueur.pion.x) <= -100:
            joueur.pion.orienter("ouest")
            joueur.peut_orienter = 0
        elif (adversaire.pion.x - joueur.pion.x) == 0 and (adversaire.pion.y - joueur.pion.y) <= -100:
            joueur.pion.orienter("nord")
            joueur.peut_orienter = 0
        else:
            joueur.pion.orienter("sud")
            joueur.peut_orienter = 0
    elif joueur.peut_deplacer >= 1:
        if joueur.pion.x - adversaire.pion.x < -100:
            joueur.pion.deplacer(100, 0, plateau, joueur)
            joueur.peut_deplacer -= 1
            ...
            joueur.pion.deplacer(0, -100, plateau, joueur)
            joueur.peut_deplacer -= 1
        elif joueur.pion.y - adversaire.pion.y < -100 and abs(joueur.pion.x - adversaire.pion.x) <= 100:
            joueur.pion.deplacer(0, 100, plateau, joueur)
            joueur.peut_deplacer -= 1
        else:
            if joueur.pion.deplacement_possible(-100, 0, plateau):
                joueur.pion.deplacer(100, 0, plateau, joueur)
                joueur.peut_deplacer -= 1
    for carte in possible:
        if carte == "deplacement" and joueur.nb_actions == 1:
            if joueur.pion.x - adversaire.pion.x < -100 :
                ...
                pile.vitesse = 1
                bouge = 1
        if (type(carte) == attaque or type(carte) == action ) and bouge == 0:
            if choix.vitesse > carte.vitesse and ((abs(joueur.pion.x - adversaire.pion.x) <= 100 and abs(joueur.pion.y - adversaire.pion.y) == 0) or (abs(joueur.pion.x - adversaire.pion.x) == 0 and abs(joueur.pion.y - adversaire.pion.y) <= 100)):
                    choix = carte
...
while game_on:
    ...
    if not joueur1_choisi :
        L_roles = [p.Rect(200, 200, 150, 210), p.Rect(1150, 200, 150, 210), p.Rect(200, 500, 150, 210), p.Rect(1150, 500, 150, 210)]
        ...
        """
        L_roles = [p.Rect(200, 200, 150, 210), p.Rect(1150, 200, 150, 210), p.Rect(200, 500, 150, 210), p.Rect(1150, 500, 150, 210)]
        texteia = police.render( "Jouer contre l'IA ", True, "black")
        screen.blit(texteia, (1312, 100))
        boutonia= p.Rect(1300,90,175,50)
        p.draw.rect(screen, (0,0,0), boutonia,width=1)
        ...
        for event in p.event.get():
            ...
            if event.type == p.MOUSEBUTTONDOWN:
                if event.button == 1:
                    ...
                    L = []
                    position = 0
                    if boutonia.collidepoint(X):
                        joueia=1
                    if L_roles[0].collidepoint(X) and lis_joueurs != [chevalier]:
                    ...
                    if joueur2_choisi:
                        effet_vide = effet("rien", "rien", "n'a pas d'effet")
                        effet_parade = effet("rien", "rien", "met fin à l'échange")
                        effet_balayage = modificateur([guerrier.droit_orientation], "carte_jouee",
                                                      "Orientez-vous. Votre prochaine carte joué perd 1 de vitesse",
                                                      [attaque, action], 0, 0, -1)
                        effet_poignard = effet([autre_joueur(assassin).saigner], "rien",
                                               "l'adversaire gagne une carte saignement")
                        effet_avancee = effet([moine.droit_deplacement], "rien", "vous vous déplacez d'une case")
                        effet_sauvage = effet([assassin.droit_deplacement, assassin.droit_deplacement], "rien",
                                              "vous vous déplacez de deux cases")
                        effet_abandon = effet([autre_joueur(moine).saigner], "rien",
                                              "l'adversaire gagne une carte saignement")
                        effet_tourbillon = effet([assassin.droit_orientation], "rien", "vous vous orientez")
                        effet_embrocher = effet([autre_joueur(chevalier).saigner, autre_joueur(chevalier).saigner],
                                                "rien", "l'adversaire gagne deux cartes saignement")
                        effet_elan = modificateur("rien", "carte_jouee",
                                                  "Votre prochaine attaque inflige 2 dégâts supplémentaires et sa vitesse augmente de 2",
                                                  [attaque], 2, 0, 2)
                        effet_foi = effet(
                            [chevalier.soin, chevalier.soin, chevalier.soin, chevalier.soin, chevalier.soin,
                             chevalier.pioche], "rien", "Vous gangez 5 PV. Piochez une carte")
                        effet_berserker = modificateur([guerrier.pioche], "fin_tour",
                                                       "jusqu'à la fin du tour, vos cartes attaque infligent 2 dégâts supplémentaires et leur vitesse augmente de 1. Piochez une carte",
                                                       [attaque], 2, 0, 1)
                        effet_egorgement = effet([autre_joueur(assassin).saigner, autre_joueur(assassin).saigner],
                                                 "rien", "l'adversaire gagne deux cartes saignement")
                        effet_transe = modificateur([moine.pioche], "fin_tour",
                                                    "jusqu'à la fin du tour, vos attaques infligent 2 dégâts supplémentaires. Piochez une carte",
                                                    [attaque], 2, 0, 0)
                        effet_destructrice = effet(
                            [autre_joueur(moine).degats, autre_joueur(moine).degats, autre_joueur(moine).degats],
                            "rien", "met fin à l'échange. L'adversaire subit 3 dégâts")
                        effet_eclair = effet([chevalier.orientation], "rien", "orientez-vous")

                        Revers_devastateur = attaque("Revers devastateur", 2, 6, 1, ["devant", "gauche", "droite"],
                                                     effet_vide, guerrier, rev)
                        Frappe_veloce = attaque("Frappe véloce", 4, 2, 1, ["devant", "gauche", "droite"], effet_vide,
                                                guerrier, vel)
                        Parade_guer = action("Parade", 4, effet_parade, guerrier, par_guer)
                        Balayage = attaque("Balayage", 4, 4, 1, ["devant", "gauche", "droite"], effet_balayage,
                                           guerrier, bal)
                        Dans_ta_face = attaque("Dans ta face", 5, 1, 1, ["devant", "gauche", "droite"], effet_vide,
                                               guerrier, face)
                        Berserker = action("Berserker", 5, effet_berserker, guerrier, bers)
                        Executer = attaque("Executer", 1, 8, 1, ["devant"], effet_vide, guerrier, exec)
                        Parade_assa = action("Parade", 4, effet_parade, assassin, par_assa)
                        Coup_de_poignard_leger = attaque("Coup de poignard léger", 4, 1, 1,
                                                         ["devant", "gauche", "droite"], effet_poignard, assassin,
                                                         poign)
                        Charge_sauvage = action("Charge sauvage", 4, effet_sauvage, assassin, sauva)
                        Frappe_verticale = attaque("Frappe verticaale", 2, 5, 1, ["devant"], effet_vide, assassin,
                                                   verti)
                        Tourbillon_de_lames = attaque("Tourbillon de lames", 4, 2, 1,
                                                      ["devant", "gauche", "droite", "derrière"], effet_tourbillon,
                                                      assassin, tourbi)
                        Egorgement = attaque("Egorgement", 1, 5, 1, ["devant", "gauche", "droite"], effet_egorgement,
                                             assassin, egor)
                        Frappe_d_estoc = attaque("Frappe d'estoc", 2, 4, 2, ["devant", "gauche", "droite"], effet_vide,
                                                 chevalier, estoc)
                        Parade_chev = action("Parade", 4, effet_parade, chevalier, par_chev)
                        Embrocher = attaque("Embrocher", 2, 5, 1, ["devant"], effet_embrocher, chevalier, embro)
                        Prendre_son_elan = action("Prendre son élan", 3, effet_elan, chevalier, elan)
                        Foi_renouvelee = action("Foi renouvelée", 4, effet_foi, chevalier, foi)
                        Frappe_eclair = attaque("Frappe éclair", 4, 4, 1, ["gauche", "droite", "derrière"],
                                                effet_eclair, chevalier, eclair)
                        High_kick = attaque("High Kick", 3, 4, 1, ["devant", "gauche", "droite"], effet_vide, moine,
                                            high)
                        Uppercut = attaque("Uppercut", 4, 6, 0, ["devant", "gauche", "droite"], effet_vide, moine, upp)
                        Frappe_du_poing = attaque("Frappe du poing", 4, 2, 1, ["devant", "gauche", "droite"],
                                                  effet_vide, moine, poing)
                        Parade_moin = action("Parade", 4, effet_parade, moine, par_moin)
                        Avancee_fulgurante = action("Avancée fulgurante", 5, effet_avancee, moine, avan)
                        Abandon_du_fair_play = attaque("Abandon du fair-play", 3, 3, 1, ["devant", "gauche", "droite"],
                                                       effet_abandon, moine, aban)
                        Transe_du_combattant = action("Transe du combattant", 4, effet_transe, moine, trans)
                        Parade_destructrice = action("Parade destructrice", 5, effet_destructrice, moine, destru)

                        deck_guer.cartes = 3 * [Revers_devastateur] + 4 * [Frappe_veloce] + 4 * [Parade_guer] + 3 * [
                            Balayage] + 3 * [Dans_ta_face] + 2 * [Berserker] + 1 * [Executer]
                        deck_assa.cartes = 4 * [Parade_assa] + 4 * [Coup_de_poignard_leger] + 2 * [
                            Charge_sauvage] + 3 * [Frappe_verticale] + 2 * [Tourbillon_de_lames] + 3 * [Egorgement]
                        deck_chev.cartes = 3 * [Frappe_d_estoc] + 4 * [Parade_chev] + 3 * [Embrocher] + 3 * [
                            Prendre_son_elan] + 2 * [Foi_renouvelee] + 3 * [Frappe_eclair]
                        deck_moin.cartes = 3 * [High_kick] + 2 * [Uppercut] + 3 * [Frappe_du_poing] + 3 * [
                            Parade_moin] + 2 * [Parade_destructrice] + 2 * [Avancee_fulgurante] + 3 * [
                                               Abandon_du_fair_play] + 2 * [Transe_du_combattant]
            ...
            if event.type == p.MOUSEBUTTONDOWN:
                ...
                if event.button == 1:
                    ...
                    if dep.collidepoint(X):
                        joueur1.degats(joueur1.saignement)
                        joueur2.degats(joueur2.saignement)
